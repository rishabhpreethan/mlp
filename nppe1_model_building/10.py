...
grid_search_pipeline = GridSearchCV(estimator=pipeline_pca, param_grid=param_grid_pipeline, cv=5, scoring='neg_mean_absolute_error', n_jobs=-1)
...
# Fit PCA to find the explained variance ratio
pca = PCA()
pca.fit(X_train)

# Get the explained variance of the first principal component
explained_variance_first_pc = pca.explained_variance_ratio_[0]
print("Variance explained by the first principal component:", explained_variance_first_pc)
...
# Predict on the test dataset using the pipeline
y_pred_poly = pipeline_poly.predict(X_test)

# Calculate the R^2 score on the test dataset
r2_poly = r2_score(y_test, y_pred_poly)
print("R^2 score on the test dataset using the PolynomialFeatures and Lasso pipeline:", r2_poly)

# Perform Recursive Feature Elimination (RFE) with LinearRegression
linear_regressor = LinearRegression()
rfe = RFE(estimator=linear_regressor, n_features_to_select=len(X_train.columns)-1)
rfe.fit(X_train, y_train)

# Find the index of the eliminated feature
eliminated_feature_index = np.where(rfe.support_ == False)[0][0]
print("Index of the eliminated feature:", eliminated_feature_index)
